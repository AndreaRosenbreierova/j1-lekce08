...
public class Svatky {
  ...
  public Stream<String> muzi() {
    ...
    return seznamSvatku()
            .filter(svatek -> svatek.getGender() == Gender.MUZ).map(Svatek::getJmeno);
  }
  ...
  public Stream<String> zeny() {
    ...
    return seznamSvatku()
            .filter(svatek -> svatek.getGender() == Gender.ZENA).map(Svatek::getJmeno);
  }
  ...
  public Stream<String> den(MonthDay den) {
    return seznamSvatku()
            .filter(svatek -> svatek.getDen().equals(den)).map(Svatek::getJmeno);
  }
  ...
  public Stream<String> zenskaJmenaVMesici(Month mesic) {
    //TODO
    return seznamSvatku()
            .filter(svatek -> svatek.getDen().getMonth() == mesic && svatek.getGender() == Gender.ZENA).map(Svatek::getJmeno);
  ...
  public int pocetMuzuSvatekPrvniho() {
    ...
    return (int) seznamSvatku()
            .filter(svatek -> svatek.getDen().getDayOfMonth() == 1 && svatek.getGender() == Gender.MUZ).count();
  ...
  public void vypsatJmenaListopad() {
    ...
    seznamSvatku()
            .filter(svatek -> svatek.getDen().getMonth() == Month.NOVEMBER).map(Svatek::getJmeno).forEach(System.out::println);
  }
...
return
        (int) seznamSvatku()
                .filter(svatek -> svatek.getGender() == Gender.MUZ || svatek.getGender() == Gender.ZENA)
                .map(Svatek::getJmeno).count();
  }
  ...
  public Stream<String> cervenJmenaOdDesatehoJmena() {
    return
    seznamSvatku()
            .filter(svatek -> svatek.getDen().getMonth() == Month.JUNE).map(Svatek::getJmeno).skip(10);
  }
  ...
  public Stream<String> jmenaOdVanoc() {
    return
            seznamSvatku()
                    ...
                    .dropWhile(svatek -> svatek.getDen().getDayOfMonth() < 24).map(Svatek::getJmeno);
  }
